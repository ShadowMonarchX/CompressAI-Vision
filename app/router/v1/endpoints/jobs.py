...
async def job(job_id: str, store=Depends(get_job_store)) -> Job:
    ...
    return Job(job_id=j.job_id, status=j.status.value, metrics=j.metrics, error=j.error, download_url=f"/api/v1/jobs/{job_id}/download" if j.status.value == "done" else None)
