...
from app.deps import get_job_store, get_settings
...
async def image(file: UploadFile = File(...), settings=Depends(get_settings)) -> JobSubmission:
    """Persist an image and enqueue the image-specific background pipeline."""
    source = await save_upload(file, settings)
    return JobSubmission(job_id=await create_job(source), status="queued")


@router.post('/video',response_model=JobSubmission,status_code=202)
async def video(file: UploadFile = File(...), settings=Depends(get_settings)) -> JobSubmission:
    ...
    source = await save_upload(file, settings)
    return JobSubmission(job_id=await create_job(source, "video"), status="queued")
